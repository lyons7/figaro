...
def recommendations():
    ...
    composer = str(request.form['ending'])
...
for i in predict_list:
    ...
    # Help from here https://stackoverflow.com/questions/37077599/find-similarity-between-two-dataframes-of-different-lengths
    jd = lambda x, y: 1 - distance.jaccard(x, y)

    test_df = pd.concat([df.iloc[:, 0] for df in [predicts, new_df]], axis=1, keys=['ending', 'kid', 'length', 'composer', 'language', 'story', 'month'])
